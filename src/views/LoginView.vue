...
const handleLogin = async () => {
  ...
  try {
    // Obtener el token
    const response = await AuthenticationService.loguearse({
    ...
    if (token) {
      ...
      const userResponse = await AuthenticationService.obtenerMunicipio(); // Rol del usuario
      const userRole = userResponse.data.role;

      // Redirigir según el rol del usuario
      if (userRole === 'Superadmin' || userRole === 'Administrador de Municipio') {
        router.push('/municipio');
      } else {
        errorMessage.value = 'No tienes permisos para acceder a esta aplicación.';
        setTimeout(() => {
          errorMessage.value = '';
        }, 3000);
      }
